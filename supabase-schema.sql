...
CREATE POLICY "Managers can create/update/delete orders in their org" ON orders
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() 
...
CREATE POLICY "Users can create/update buy list items in their org" ON buy_list
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() 
